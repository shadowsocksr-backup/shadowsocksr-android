...
class Profile {
  ...
  override def toString = "ssr://" + Base64.encodeToString("%s:%d:%s:%s:%s:%s/?obfsparam=%s&remarks=%s".formatLocal(Locale.ENGLISH,
    ...
    password).getBytes, Base64.NO_PADDING | Base64.URL_SAFE), Base64.encodeToString("%s".formatLocal(Locale.ENGLISH,
    obfs_param).getBytes, Base64.NO_PADDING | Base64.URL_SAFE), Base64.encodeToString("%s".formatLocal(Locale.ENGLISH,
    name).getBytes, Base64.NO_PADDING | Base64.URL_SAFE)).getBytes, Base64.NO_PADDING | Base64.URL_SAFE)
